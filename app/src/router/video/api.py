...
async def get_videos(
    ...
    with response_handler() as response:
        ...
        response.message = t("video_list_success", lang)
...
async def claim_point(
    ...
    with response_handler() as response:
        ...
        if not video:
            raise ValueError(t("video_not_found", lang))
        reward = await crud_reward.get_by_user_id_video_id(session=session, video_id=video_id, user_id=user_id)
        if not reward:
            await crud_reward.create(
                session=session,
                video_reward_claim=VideoRewardClaim(**{"video_id": video_id, "user_id": user_id}),
            )
